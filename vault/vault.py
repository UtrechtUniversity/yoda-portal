...
@vault_bp.route('/yda/<yoda_id>')
def resolve(yoda_id):
    # Check if Yoda ID is a valid UUID4.
    try:
        if UUID(yoda_id).version != 4:
            abort(404)
    except ValueError:
        abort(404)

    # Find data package with provided Yoda ID.
    response = api.call('vault_get_package_by_id', {"yoda_id": yoda_id})
