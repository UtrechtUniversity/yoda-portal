 ...
 */
DataTable.Api.register('page.jumpToData()', function (data, column) { // FIXME: Missing column
	const table = this;
	const cacheInfo = getFolderContents.getCache(); // Get cached data
  
	// Check if cache is valid for the current folder/sort
	if (cacheInfo.cacheFolder !== currentFolder ||
	  ...
	  return table.ajax.reload(); // Optional: Force reload
	}
  
	// Search the cached data (not DataTables' internal data)
	const pos = cacheInfo.cache.findIndex(item => item.name === data); //TODO: Original codes (JTD)
  
	if (pos >= 0) {
	  // Calculate the position within the cached batch
	  const globalPos = cacheInfo.cacheStart + pos;
	  const page = Math.floor(globalPos / table.page.info().length); 
	  table.page(page).draw(false); // TODO: original codes (JTD)
	  console.log(`Jumped to page ${page} (position ${globalPos})`);
	} else {
	  console.log('Data not in cached batch. Server-side search.');
	}
